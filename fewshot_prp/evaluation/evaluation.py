...
class ModelEvaluation:
    ...
    def evaluate(r_model, metrics: list = [AP(rel=2)@100, NDCG(cutoff=10)], fusion_sum: bool  = False, w1: float  = 0.7, w2: float  = 0.3, fallback: bool  = False, reciprocal_rank: bool = False):
        scores_df_sorted = pd.DataFrame()
        sys.path.append("..")
        if r_model=='baselines':
            files = [f for f in os.listdir(f'./datasets/phase_one_retrieval/') if re.match(r'ranked*', f)]
        else:
            files = [f for f in os.listdir(f'./scores/') if re.match(r'reranking_scores*', f)]
        ...
        for file in files:
            ...
            if fusion_sum==True and r_model!='baselines':
                ...
                eval_dfusion_sum = Scoring.direct_fusion_sum(metrics, ori, res, qrels, w1, w2)
                #eval['Fusion Sum'] = eval_fusion_sum
                eval['DFusion Sum'] = eval_dfusion_sum

            # Fallback mode
            if fallback==True and r_model!='baselines':
                if ds=='covid' or ds=='touche' or ds=='toucheV2' or ds=='scifact':
                    pd.options.mode.chained_assignment = None
                    ori_name = f'ranked_bm25_{ds}_t{top_k}.csv'
                    ori = pd.read_csv(f'./datasets/phase_one_retrieval/{ori_name}')
