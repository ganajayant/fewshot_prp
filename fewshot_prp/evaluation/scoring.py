...
class Scoring:
    ...
    def direct_fusion_sum(metrics, res_ori, res_zer, qrels, w1:float = 1.0, w2:float = 1.0):
        pd.options.mode.chained_assignment = None  # default='warn'
    ...
    def fallback(metrics, res_ori, res_zer, qrels):
        ...
        for qid in unique_qids:
            ...
            for score in unique_scores:
                ...
                for docid in ranking_qid_sub.docid:
                    original_score = original['score'].loc[original['docid']==docid].values[0]
        ...
        res_fall_total['score'] = max(res_fall_total['updated_score']) - res_fall_total['updated_score']
    ...
    def reciprocal_rank_topics(metrics, topics, ds, top, res_zer, qrels):
        ...
        new_res_zer[['qid', 'docid']] = new_res_zer[['qid', 'docid']].apply(pd.to_numeric)

        # scoring rr
        index = pt.IndexFactory.of(configuration.datasets[ds]['index'], memory=True)
        bm25 = pt.BatchRetrieve(index, controls={"wmodel": "BM25", "bm25.b" : configuration.bm25_b, "bm25.k_1": configuration.bm25_k1}, metadata=["docno", "text"])%top
        ...
        for index, row in res_rr.iterrows():
            ...
            print('------------------------------')
            rank_llm = new_res_zer['rank'].loc[(new_res_zer['qid']==qid)&(new_res_zer['docid']==docid)].values[0]
            rr_score = math.log(1+(1/(rank+1))) + math.log(1+(1/(rank_llm+1)))
    ...
    def reciprocal_rank(metrics, res_original, res_zer, qrels, ds):
        ...
        for index, row in res_rr.iterrows():
            ...
            try:
                rank_llm = current_res['rank'].loc[new_res_zer['docno'].astype(str)==str(docid)].values[0]
            except:
                pass
            rr_score = math.log(1+(1/(rank+1))) + math.log(1+(1/(rank_llm+1)))
            res_rr.loc[index, 'score'] = rr_score
