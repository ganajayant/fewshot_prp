...
class RelevancyScorerLLM:
    ...
    def transform(self, topics_or_res: pd.DataFrame, few_shot_examples: list):
        res = {
            "qid": [],
            "query": [],
            "docid": [],
        ...
        prediction_logs = []
        with torch.no_grad():
            for (qid, query), query_results in tqdm(
                topics_or_res.groupby(["qid", "query"]), unit="q"
            ):
                # for all-pair processing:
                self.parameters["query_num"] = query_num
                doc_no, doc_ids, doc_texts, scores = AllPair(
                    self.parameters, qid, query, query_results, few_shot_examples
                )
                query_num += 1
    ...
    def __init__(
        ...
        topics_or_res = pd.read_csv(
            data_path + f"phase_one_retrieval/ranked_{r_model}_{ds}_t{top_k}.csv"
        )
        self.parameters = {}
        ...
        if reranker == "zephyr":
            self.A = self.tokenizer.encode(
                "1", return_tensors="pt", add_special_tokens=False
            )[0][1]
            self.B = self.tokenizer.encode(
                "2", return_tensors="pt", add_special_tokens=False
            )[0][1]
        else:
            self.A = self.tokenizer.encode(
                "1", return_tensors="pt", add_special_tokens=False
