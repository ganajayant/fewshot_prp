...
def check_file(reranker,ds,top_k,mode,kshot,random_seed):
    sys.path.append("..")
    res_files = [f for f in os.listdir(f'./scores/') if re.match(r'reranking_scores*', f)]
    ...
    if current_file in res_files:
        print(f'SKIPPING EXPERIMENT : {ds}_t{top_k}_{reranker}_{mode}{configuration.max_fewshot}_R{random_seed}_{kshot}-shot')
        return True
    else:
        print(f'RUNNING EXPERIMENT : {ds}_t{top_k}_{reranker}_{mode}{configuration.max_fewshot}_R{random_seed}_{kshot}-shot')
        return False

def check_res(r_model,ds,top_k):
    sys.path.append("..")
    res_files = [f for f in os.listdir(f'./datasets/phase_one_retrieval/') if re.match(r'ranked_*', f)]
    
    current_file = f'ranked_{r_model}_{ds}_t{top_k}.csv'
    
    if not current_file in res_files:
            ...
            for top_k in top_ks:
                for kshot in kshots:
                    if kshot==0:
                        ...
                        random_seed = 42
                        if not check_file(reranker,ds,top_k,mode,kshot,random_seed):
                            # Run the RelevancyScorerLLM class with all the parameters
                            check_res(r_model,ds,top_k)
                            RelevancyScorerLLM(model, tokenizer, reranker, top_k, ds, kshot, random_seed, mode, eval)
                    else:
                        # few-shot
