from .text import few_shot, mini_few_shot, mini_zero_shot, zero_shot
...
class ROFewShotPrompt:
    def __new__(self, query: str, doc1: str, doc2: str, examples: list) -> str:
        ...
        for example in examples:
            example_format = (
                '''
            A passage relevant to the query "'''
                + str(example["rel_query_" + str(counter)])
                + '''" is given.
            Relevant Passage: "'''
                + str(example["rel_doc_" + str(counter)])
                + """"
            """
            )
        ...
        return few_shot.format(
            query=query, doc_one=doc1, doc_two=doc2, examples=final_examples
        )
        ...
        for example in examples:
            if counter % 2 == 0:
                ...
                Passage 1 : "'''
                    ...
                    + '''"
                Passage 2 : "'''
                    + str(example["nrel_doc_" + str(counter)])
                    + """"
                Output: 1
    ...
    def __new__(self, query: str, doc1: str, doc2: str, examples: list) -> str:
        ...
        for example in examples:
            if counter % 2 == 0:
                example_format = (
                    '''
                Which of the following two passages is more relevant to "'''
                    + str(example["rel_query_" + str(counter)])
                    + '''"?
                1: "'''
                    + str(example["rel_doc_" + str(counter)])
